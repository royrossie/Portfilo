...
/* 
	...
	What differences:
		changed code: instead save H matrix to specail unreadable file, we save matrix to H_MATRIX file in our directory
		added code: also we save H matrix to my_H external variable, defined in "program.h" file
*/
...
( FILE *f,
  mod2dense *m, int ones
)
{ 
  int i, j;

	/*
		Additional code: instead save H matrix to specail unreadable file, we save matrix to H_MATRIX file in our directory
	*/
    FILE *special_file;									
	char my_name_file[100];
	sprintf (my_name_file, "matrix.dat");
    special_file = fopen(my_name_file, "w");					//H_MATRIX

  for (i = 0; i<mod2dense_rows(m); i++)
  { for (j = 0; j<mod2dense_cols(m); j++) {
 //    fprintf(f," %d",mod2dense_get(m,i,j));	
		my_H[i][j] = mod2dense_get(m,i,j);
	fprintf(special_file,"%d",mod2dense_get(m,i,j));	
    }
//    fprintf(f,"\n");									
  fprintf(special_file,"\n");					
  }
   fclose(special_file);
}
