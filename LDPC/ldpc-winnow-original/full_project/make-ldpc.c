...
/* 
	...
	What differences:
		...
		main function changed name to "ldpc";
		instead "argc" and "argv" arguments function receives exactly:
			pchk-file, n-checks n-bits seed method checks-per-col (without "no4cycle" now);	
		deleted "Look at initial arguments";
		deleted variable "meth", instead it uses exactly new variables: main_method, main_ones;
*/
...
int ldpc (char *main_ldpc_file, int main_rows, int main_columns, int main_seed, char *main_method, char *main_ones/*, char *main_extra_method*/) {
  make_method method;
  char *file;
  int seed, no4cycle;
  distrib *d;
  ...
  FILE *f;
	...
	/*
		Chaged code: instead to look initial arguments we write extra arguments from main program to variables
	*/
  ...
  if (strcmp(main_method,"evencol")==0 || strcmp(main_method,"evenboth")==0)
  { method = strcmp(main_method,"evencol")==0 ? Evencol : Evenboth;
    if (!main_ones)
    { usage();
    }
    d = distrib_create(main_ones);
    if (d==0)
    { usage();
    }
 /*   if (main_extra_method)							//HERE
    { if (strcmp(main_extra_method,"no4cycle")==0)
      { no4cycle = 1;
      }
      else
      { usage();
